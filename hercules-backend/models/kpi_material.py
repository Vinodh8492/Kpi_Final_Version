...
class KPIMaterial(db.Model):
    ...
    id = db.Column('id', db.Integer, primary_key=True)
    batch_guid = db.Column('batch_guid', db.String(100))
    batch_name = db.Column('batch_name', db.String(255))
    product_name = db.Column('product_name', db.String(255))
    batch_act_start = db.Column('batch_act_start', db.DateTime)
    batch_act_end = db.Column('batch_act_end', db.DateTime)
    quantity = db.Column('Quantity', db.Float)
    material_name = db.Column('material_name', db.String(255))
    material_code = db.Column('material_code', db.String(100))
    setpoint_float = db.Column('setpoint_float', db.Float)
    actual_value_float = db.Column('actual_value_float', db.Float)
    source_server = db.Column('source_server', db.String(100))
    rootguid = db.Column('ROOTGUID', db.String(100))
    order_id = db.Column('OrderId', db.String(100))
    event_id = db.Column('EventID', db.String(100))
    batch_transfer_time = db.Column('batch_transfer_time', db.String(100))
    formula_category_name = db.Column('FormulaCategoryName', db.String(255))
